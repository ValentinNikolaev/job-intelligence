...
1. Read `config/codex-workflows.yaml`. Confirm the task uses either `prepare` or `prepare_priority` exactly as requested. Use CLI spelling `prepare-priority` for the latter.
2. Run `python run.py pending prepare all --workflow <prepare|prepare-priority>`.
3. Select only the first printed vacancy. The deterministic command already excludes unanalyzed vacancies and routes normal scores from `prepare_min_score` through `priority_score - 1`, while priority scores start at `priority_score`. If no path is printed, report that this queue is current and stop.
...
7. Publish with `python run.py prepare <vacancy-directory> --input <draft-directory> --workflow <prepare|prepare-priority>`.
