# Application Manager (Payment) (wmd)

Posted: 2026-07-27T08:30:30Z

Über uns

Bei G+D Netcetera entwickeln wir digitale Lösungen für die Finanzbranche, die dazu beitragen, Bezahlprozesse für Millionen von Menschen weltweit einfach, sicher und zuverlässig zu gestalten. Mit unserer Arbeit im Bereich Banking und Payments unterstützen wir die Infrastruktur hinter alltäglichen Transaktionen und leisten einen Beitrag zur digitalen Zukunft der Finanzwelt.

Unsere Teams arbeiten in Bereichen wie Banking & Payments, Cards und Trusted Solutions und entwickeln Technologien, die sichere und nahtlose digitale Erlebnisse ermöglichen. Gleichzeitig fördern wir den Wissensaustausch in der Branche – unter anderem durch Publikationen, Expertenbeiträge und Fachveranstaltungen.

## Aufgaben

- Fachliche Betreuung von Anwendungen zur Prüfung und Freigabe von neuen Kartenprodukten nach DK-Spezifikationen

- Pflege kartenspezifischer Parameter in den entsprechenden Anwendungen.

- Durchführung, Koordination und Dokumentation von Kartentests (SRC Testsuite2, ISS), sowie fachlich-technische Datenprüfung

- Steuerung und Koordination externer Dienstleister und Partner, sowie Mitarbeit in Gremien

- Verantwortung für die Koordination von Zulassungsprozessen unter Berücksichtigung regulatorischer und fachlicher Vorgaben (z. B. AK-Zulassung, DK, Mastercard, S-Payment)

- Umgang mit kryptografischen Schlüsseln, Zertifikaten und Verschlüsselungsmechanismen im Rahmen von Testprozessen

## Qualifikation

- Abgeschlossenes (Fach-)Hochschulstudium der Informatik oder vergleichbare Ausbildung

- Erfahrung im Softwaretest, idealerweise im agilen Entwicklungsumfeld

- Fundierte Kenntnisse und Anwendung von Qualitätsmanagementprinzipien, inklusive einer starken Kundenorientierung und einem prozessorientierten Ansatz

- Ausgeprägte Leistungsbereitschaft, großes Verantwortungsbewusstsein, starkes Organisationsmanagement und ausgezeichnete Teamfähigkeit

- Sehr gute Deutsch- und gute Englischkenntnisse

## Benefits

- **Events und Community**: Von Team-Events wie Sommerfest und Weihnachtsfeier bis zu sportlichen Aktivitäten (B2Run) – bei uns erlebst du ein starkes Miteinander

- **Individuelle Entwicklungsmöglichkeiten**: Regelmäßige Trainings und Weiterbildungen, die dich voranbringen

- **Flexible Arbeitszeiten** und Möglichkeiten es mobilen Arbeitens, um Beruf und Privatleben optimal zu verbinden

- **30 Urlaubstage** und 3,5 Brauchtumstage für deine Erholung

- **Office-Ausstattung:** Ergonomische Arbeitsplätze und kreative Workspaces, in denen du dich wohlfühlst

- **Gesunde Ernährung**: Bezuschusstes, ausgewogenes Mittagessen mit Fokus auf Nachhaltigkeit

- **Mobilität:** Deutschlandticket und Bike-Leasing für deinen täglichen Arbeitsweg

- **Sport und Vorsorge**: Urban Sports Club, Grippeschutzimpfung, Krankenzusatzversicherung, für deine Gesundheit

- **Rabatte**: Mitgliedschaft bei Corporate Benefits u.v.m.

Find [Jobs in Germany](https://www.arbeitnow.com) on Arbeitnow
