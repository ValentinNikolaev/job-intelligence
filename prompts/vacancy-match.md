...
- Apply these candidate-specific scoring preferences after judging the evidence: decrease the score for roles that are not remote or do not clearly allow remote work; decrease the score when Spring Boot is a central requirement; decrease the score when ASP.NET, ASP.NET Core, or .NET web backend experience is a central requirement; decrease the score for Site Reliability Engineer or SRE roles because they are usually closer to DevOps than software development; increase the score when the role offers a relocation package; increase the score when PHP is a meaningful part of the role; increase the score when Go or Golang is a meaningful part of the role; increase the score when the role is based in Roma or Rome; increase the score when the role involves support automation; increase the score when the role is in the mail/email domain; increase the score when the role is in the support domain.
- Treat an explicit US-only timezone, California/San Francisco timezone, U.S.-based hours, or required overlap with U.S.-based teams as a decisive location/timezone conflict unless the vacancy also clearly allows working from European, EU, EMEA, or candidate-local time zones.
