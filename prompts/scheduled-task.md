...
This task performs no model-dependent vacancy work, so configure it with GPT-5.6 Luna and low reasoning. Analysis and preparation are separate tasks because their configured models differ. Run it in Local mode so the ignored `sources/.env` remains available.
