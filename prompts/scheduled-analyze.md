...
when the command exits with code 0 and prints either `Telegram notification sent.` or
`Telegram notification already sent; duplicate skipped.`;
...
If Telegram is unavailable or the send fails, preserve any unsent message in a collision-resistant `.codex-work/telegram-message-<unique-prefix>.txt` file; never overwrite another process's message. Always invoke `python scripts/notify_telegram.py` exactly once per union message and let it load `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` from the task/host environment or the ignored `sources/.env`; do not invoke it again merely to capture or inspect its output, and do not pre-skip based only on the current process environment. The sender records a local delivery receipt and suppresses an identical repeat for the same Telegram chat. If this task sends a Telegram message for new vacancy analysis, include exactly one initiator line, `Automation ID: job-intelligence-batch-vacancy-analysis`, followed by one block per vacancy in this exact field order: `<Company> — <Title>`, `Score: <score>`, `Vacancy ID: <vacancy_id>`, `Directory: <directory>`, `URL: <source_url>`. Put one empty line between vacancy blocks. Do not include `Internal initiator:` or prose labels as initiator text. Use `vacancy_id`, `directory`, and `source_url` from `python run.py api queues analyze --json` or the sealed analysis pack. Never write `URL: unavailable` when the queue, pack, or selected vacancy `meta.yaml` contains a non-empty source URL. The sender fails closed and preserves the draft if any vacancy URL remains missing after registry enrichment. The sender applies safe Telegram HTML styling to headlines and labels, so keep the draft as plain text without Markdown or HTML tags.
